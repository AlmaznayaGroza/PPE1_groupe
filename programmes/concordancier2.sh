...
# Créer le dossier de sortie pour les concordanciers s'il n'existe pas
mkdir -p ./concordances

# Parcourir tous les fichiers .txt dans le dossier
for FILE in "$DOSSIER"/*.txt; do
    if [[ ! -f "$FILE" ]]; then
        echo "Aucun fichier texte trouvé dans le dossier $DOSSIER."
        continue
    fi
...
<body>
    ...
    echo "Concordanciers générés pour $BASENAME :"
    echo "  - $OUTPUT_FILE1"
    echo "  - $OUTPUT_FILE2"
